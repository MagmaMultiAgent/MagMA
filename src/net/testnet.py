...
import numpy as np
from config import ModelParam, ActDims
...
batch_size = 1
global_feature_shape = (64,)
map_feature_shape = (64, 16, 16)



action_feature_shape = (64, 8)

# Create random input tensors
global_feature = torch.randn(batch_size, *global_feature_shape)
map_feature = torch.randn(batch_size, *map_feature_shape)
action_feature = torch.randn(batch_size, *action_feature_shape)

va = torch.randn(batch_size, 1)

# Create the model_param object
model_param = ModelParam()
model = Net(model_param)

# Pass the input tensors through the network
logp, critic_value, action, entropy, teacher_action_logp = model(global_feature, map_feature, action_feature, va)

# Fix the indexing error
indices = torch.nonzero(action_feature)
b, x, y = indices[:, 0], indices[:, 1], indices[:, 2]

# Visualize the network using PyTorchViz
from torchviz import make_dot
make_dot((logp, critic_value, action, entropy, teacher_action_logp)).render("testnet", format="png")
